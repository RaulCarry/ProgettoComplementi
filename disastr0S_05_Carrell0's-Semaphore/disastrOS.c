...
void disastrOS_start(void (*f)(void*), void* f_args, char* logfile){  
  ...
  //sem
  syscall_vector[DSOS_CALL_SEM_OPEN]  = internal_sem_open;    
  syscall_numarg[DSOS_CALL_SEM_OPEN]  = 1;

  syscall_vector[DSOS_CALL_SEM_WAIT]  = internal_sem_wait;    
  syscall_numarg[DSOS_CALL_SEM_WAIT]  = 1;

  syscall_vector[DSOS_CALL_SEM_POST]  = internal_sem_post;    
  syscall_numarg[DSOS_CALL_SEM_POST]  = 1;

  syscall_vector[DSOS_CALL_SEM_CLOSE] = internal_sem_close;   
  syscall_numarg[DSOS_CALL_SEM_CLOSE] = 1;
  ...
  makecontext(&running->cpu_state,
              (void(*)(void))f,
              1,
              f_args);

  
  disastrOS_debug("starting init process\n");
